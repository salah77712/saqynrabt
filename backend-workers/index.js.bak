...
async function verifyClerkWebhook(request, bodyText, webhookSecret, env) {
    ...
    try {
        ...
        const key = await crypto.subtle.importKey("raw", keyData, { name: "HMAC", hash: "SHA-256" }, false, ["verify", "sign"]);
...
async function verifyJWT(authHeader, env) {
    ...
    if (env.CLERK_JWT_VERIFICATION_KEY) {
        try {
            ...
            const signatureBin = new Uint8Array(atob(parts[2].replace(/-/g, '+').replace(/_/g, '/'))
                .split("")
                .map((c) => c.charCodeAt(0)));
            const isValid = await crypto.subtle.verify("RSASSA-PKCS1-v1_5", publicKey, signatureBin, data);
            if (!isValid)
                return null;
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (!env.OPENAI_API_KEY || !env.DATABASE_URL || !env.PINECONE_API_KEY || !env.CLERK_SECRET_KEY) {
            ...
            return new Response(JSON.stringify({ error: 'System configuration error. Please contact admin.', requestId }), { status: 500, headers: { ...headers, 'Content-Type': 'application/json' } });
        }
          ...
          `;
            ...
            `;
                        ...
                        if (activeCount >= maxEmployees) {
                            return new Response(JSON.stringify({ error: 'LIMIT_REACHED', message: 'Plan limit reached. Upgrade to add more team members.' }), { status: 403, headers: { ...headers, 'Content-Type': 'application/json' } });
                        }
                    }
                    await sql `
            UPDATE company_members
        ...
        `;
            ...
            if (url.pathname === '/api/automation' && request.method === 'POST') {
                ...
                if (limitCheck.limitReached) {
                    return new Response(JSON.stringify({ error: 'LIMIT_REACHED', message: 'Monthly text request limit reached. Enable auto-overage or upgrade plan.', requestId }), { status: 429, headers: { ...headers, 'Content-Type': 'application/json' } });
                }
        ...
        `;
            ...
            if ((url.pathname === '/api/documents' || url.pathname === '/api/ingest') && request.method === 'POST') {
                ...
                if (file.size > 10 * 1024 * 1024) {
                    return new Response(JSON.stringify({ error: 'LIMIT_EXCEEDED', message: 'PDF size exceeds the 10MB limit.' }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
                }
        ...
        `;
                ...
                if (activeDocsCount >= maxDocs) {
                    return new Response(JSON.stringify({ error: 'LIMIT_REACHED', message: 'Plan limit reached. Upgrade to add more documents.' }), { status: 403, headers: { ...headers, 'Content-Type': 'application/json' } });
                }
