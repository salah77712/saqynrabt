...
export function OrgChart() {
  ...
  const renderNode = (node: EmployeeNode) => {
    return (
      <div key={node.name} className="flex flex-col items-center mt-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm w-48 text-center">
          <p className="text-xs font-bold text-navy">{node.name}</p>
          <p className="text-[10px] text-muted font-bold mt-1 uppercase tracking-wider">{node.role}</p>
        </div>
  ...
  return (
    <div className="bg-slate-50 border border-gray-200/80 rounded-2xl p-6 overflow-x-auto w-full" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      <h3 className="text-base font-bold text-navy mb-1 text-center">
        {t('Organizational Reporting Tree', 'شجرة الهيكل التنظيمي للموظفين')}
      </h3>
      <p className="text-[10px] text-muted font-semibold text-center mb-6">
