...
export function Sidebar({ isCollapsed, onToggle, pendingCount = 0 }: SidebarProps) {
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-800 flex flex-col justify-between transition-all duration-300 md:static h-full ${
        isCollapsed ? 'w-20' : 'w-64'
      }`}
    >
      <div className="flex flex-col flex-1 overflow-y-auto">
        <div className="px-6 py-6 border-b border-gray-50 dark:border-slate-800 flex items-center justify-between min-h-[80px]">
          <Link href="/" className="font-extrabold text-navy dark:text-white text-lg">
            ...
            return (
              <Link
                ...
                className={`flex items-center rounded-xl text-sm font-bold transition-all relative ${
                  ...
                  isActive
                    ? 'bg-[#141F33]/5 border-l-4 border-[#141F33] text-[#141F33] dark:text-white dark:bg-royal/10 dark:border-royal'
                    : 'border-l-4 border-transparent text-[#718096] hover:bg-slate-50 dark:hover:bg-slate-800'
                }`}
              ...
              >
                ...
                {item.badge && pendingCount > 0 && (
                  <span className={`absolute bg-red-500 text-white rounded-full text-[9px] font-bold flex items-center justify-center ${
      ...
      <div className="flex justify-end p-4 border-t border-gray-50 dark:border-slate-800">
        <button
          onClick={onToggle}
