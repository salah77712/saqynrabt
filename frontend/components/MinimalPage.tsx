...
{description && <p className="mt-6 text-lg leading-8 text-primary/70">{t(description)}</p>}

{sections && (
<div className="mt-10 space-y-8">
{sections.map((s, i) => (
<section key={i}>
<h2 className="text-2xl font-semibold text-primary">{t(s.title)}</h2>
<p className="mt-3 text-primary/70">{t(s.body)}</p>
</section>
))}
</div>
)}
...
{cta && (
<div className="mt-10">
<a href={cta.href} className="inline-flex rounded-xl bg-primary px-6 py-3 text-xs font-bold text-surface min-h-[44px] items-center transition-all duration-300 hover:shadow-md hover:scale-[1.02] active:scale-95">
{t(cta.label)}
</a>
</div>
)}
