...
export function AdaptiveTour() {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
      ...
      <div>
        <h3 className="text-xs font-extrabold text-[#718096] uppercase tracking-wider">{t({ en: 'Onboarding Checklist Progress', ar: 'تقدم قائمة متطلبات التهيئة' })}</h3>
      ...
      <div className="space-y-2 text-xs font-semibold">
        {steps.map((step) => (
          <label key={step.id} className="flex items-center gap-3 cursor-pointer p-2.5 hover:bg-slate-50 rounded-xl transition-colors">
            <input
              type="checkbox"
            ...
            />
            <span className={step.checked ? 'line-through text-slate-400 font-medium' : 'text-[#141F33]'}>
              {step.label}
            </span>
          </label>
        ))}
