  ...
  return (
    <div className="bg-[#F8F9FB] border border-[#141F33]/10 rounded-2xl p-6 shadow-sm space-y-4">
      ...
      <div className="space-y-2 text-xs font-semibold">
        {steps.map((step) => (
          <label key={step.id} className="flex items-center gap-3 cursor-pointer p-2.5 hover:bg-[#141F33]/5 rounded-xl transition-colors">
            <input
              type="checkbox"
              checked={step.checked}
              onChange={() => handleToggle(step.id)}
              className="h-4 w-4 rounded border-[#141F33]/30 text-[#141F33] focus:ring-[#141F33]"
            />
