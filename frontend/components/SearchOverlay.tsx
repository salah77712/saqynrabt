...
export function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4 search-backdrop bg-black/40"
      onClick={(e) => {
    ...
    >
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-gray-200/60 overflow-hidden animate-slideDown">
        {/* Search Input */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-navy text-sm font-medium placeholder-slate-400 outline-none min-h-[36px]"
            autoComplete="off"
            spellCheck={false}
          />
          <kbd className="hidden sm:inline-flex items-center gap-1 rounded-md border border-gray-200 bg-gray-50 px-2 py-1 text-[10px] font-bold text-muted tracking-wider">
            ESC
          </kbd>
        </div>
              ...
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <svg
                      className="h-4 w-4 text-slate-400 flex-shrink-0 rtl:rotate-180"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                      <path
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                {['Features', 'Pricing', 'Help', 'API'].map((tag) => (
                  <button
                    ...
                    className="rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-semibold text-muted hover:bg-gray-100 transition-colors min-h-[32px]"
                  >
                    {tag}
                  </button>
                ))}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-3 text-[10px] font-bold text-muted uppercase tracking-wider">
