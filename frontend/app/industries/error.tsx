...
export default function IndustriesError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
    ...
    <div className="bg-surface text-primary min-h-screen flex flex-col font-sans">
      <header className="sticky top-0 z-50 w-full border-b border-primary/10/80 bg-surface backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="ms-3"><p className="text-sm font-semibold tracking-[0.2em] text-primary">SAQYN RABT</p><p className="text-[10px] uppercase tracking-[0.35em] text-primary">Connecting teams, automating workflows</p></div>
        </div>
      </header>
      <div className="flex-1 flex items-center justify-center px-6 py-20">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-primary mb-2">{t({ en: 'Industries page unavailable', ar: 'ØµÙØ­Ø© Ø§Ù„Ù‚Ø·Ø§Ø¹Ø§Øª ØºÙŠØ± Ù…ØªÙˆÙØ±Ø©' })}</h1>
          <p className="text-primary mb-8">{t({ en: "We couldn&apos;t load the industries page. Please try again.", ar: 'ØªØ¹Ø°Ø± ØªØ­Ù…ÙŠÙ„ ØµÙØ­Ø© Ø§Ù„Ù‚Ø·Ø§Ø¹Ø§Øª. ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰.' })}</p>
