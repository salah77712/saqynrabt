  ...
  return (
    <div className="bg-[#F8F9FB] text-[#1A202C] min-h-screen flex flex-col font-sans" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <main className="flex-1 max-w-4xl mx-auto py-24 px-6 w-full space-y-12">
        {/* Header */}
        <div>
          <span className="text-xs font-extrabold tracking-widest text-[#2A5CFF] uppercase">{t({ en: 'API Portal', ar: 'منصة الـ API' })}</span>
          ...
          {endpoints.map((ep, idx) => (
            <div key={idx} className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm space-y-6">
              ...
              <div className="flex items-center gap-3">
                <span className="bg-emerald-600 text-white text-[10px] font-extrabold px-3 py-1 rounded-lg uppercase tracking-wider">
                  {ep.method}
                </span>
              ...
              <p className="text-xs font-semibold text-[#718096]">{ep.desc}</p>
