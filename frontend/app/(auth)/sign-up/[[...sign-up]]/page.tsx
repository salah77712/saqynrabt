...
export default function SignUpPage() {
  ...
  const getPasswordStrength = () => {
    if (!password) return { label: '', color: 'bg-surface', score: 0 };
    let score = 0;
    if (password.length >= 8) score++;
    if (/[A-Z]/.test(password)) score++;
    if (/[0-9]/.test(password) || /[^A-Za-z0-9]/.test(password)) score++;

    if (score === 1) return { label: t({ en: 'Weak', ar: 'ضعيف' }), color: 'bg-primary', score: 33 };
    if (score === 2) return { label: t({ en: 'Medium', ar: 'متوسط' }), color: 'bg-accent', score: 66 };
    return { label: t({ en: 'Strong', ar: 'قوي' }), color: 'bg-accent', score: 100 };
  };
    ...
    return (
      <div className="flex min-h-screen bg-surface items-center justify-center px-4" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
        <div className="bg-surface rounded-xl shadow-xl p-8 border border-primary/10 w-full max-w-md animate-fadeIn text-center">
          ...
          <p className="text-xs font-semibold text-primary/60 mb-6 leading-relaxed">
            {t({ 
...
{error && (
    <div className="bg-primary border border-primary/10 text-primary rounded-xl p-3.5 text-xs font-bold mb-6 text-center">
{error}
</div>
)}
          ...
          <form onSubmit={handleVerification} className="space-y-4">
            <input
              ...
              className="w-full min-h-[44px] bg-surface border border-primary/10 rounded-xl px-4 py-3 text-center text-sm font-bold tracking-widest text-primary focus:outline-none focus:ring-2 focus:ring-accent focus:ring-royal"
              required
            />
            <button
              type="submit"
...
{error}
        ...
        {emailParam ? (
          <div className="mb-6 p-4 rounded-xl bg-surface border border-accent/10/80 text-accent flex items-start gap-4.5 shadow-sm">
            <Lock className="w-5 h-5 text-primary shrink-0 mt-0.5" />
            <div>
              <h4 className="text-xs font-black tracking-wide uppercase text-accent">
                {t({ en: "Invited Workspace Account", ar: "حساب مساحة عمل مدعو" })}
              </h4>
              <p className="text-[11px] leading-relaxed font-semibold text-accent/90 mt-1">
                {t({
          ...
          <div className="mb-6 p-4 rounded-xl bg-surface border border-accent/10/80 text-primary flex items-start gap-4.5 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              disabled={!!emailParam}
              className="w-full min-h-[44px] bg-surface border border-primary/10 rounded-xl px-4 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-accent focus:ring-royal disabled:opacity-70 disabled:cursor-not-allowed"
              required
            />
          </div>
          ...
          {/* Password */}
          <div>
            <label htmlFor="password" className="block text-xs font-bold text-primary mb-1.5">{t({ en: 'Password', ar: 'كلمة المرور' })}</label>
            <input
              type="password"
              id="password"
            ...
            {password && (
              <div className="mt-3 flex items-center justify-between gap-4 text-xs font-bold">
                ...
                <span className="text-primary/60 uppercase shrink-0">{strength.label}</span>
              </div>
            )}
          </div>
          ...
          {/* Confirm Password */}
          <div>
            <label htmlFor="confirmPassword" className="block text-xs font-bold text-primary mb-1.5">{t({ en: 'Confirm Password', ar: 'تأكيد كلمة المرور' })}</label>
            <input
              type="password"
              id="confirmPassword"
              autoComplete="new-password"
        ...
        {/* Footer Link */}
        <p className="text-center text-xs text-primary/60 font-bold mt-8">
