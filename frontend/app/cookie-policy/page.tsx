...
const content = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: {
        en: 'These cookies are essential for the Platform to function correctly. They enable core features such as secure user authentication (via Clerk) and session state management. Without these cookies, services you have requested — such as logging into your dashboard — cannot be provided. These cookies do not track browsing activity on third-party sites and do not require prior consent, though they are subject to this policy. Specific technologies include: Clerk session tokens for authentication and Redis-backed server cache identifiers that maintain dashboard continuity across page loads. *(Note: Self-serve payment processing is currently disabled and no payment cookies are set.)*',
        ar: 'ملفات تعريف الارتباط هذه ضرورية لتشغيل المنصة بشكل صحيح. تُمكّن الميزات الأساسية مثل التحقق الآمن من هوية المستخدم (عبر Clerk) وإدارة حالة الجلسة. بدون ملفات تعريف الارتباط هذه، لا يمكن تقديم الخدمات التي طلبتها — مثل تسجيل الدخول إلى لوحة التحكم. لا تتتبع هذه الملفات نشاط التصفح على مواقع الطرف الثالث ولا تتطلب موافقة مسبقة، رغم أنها تخضع لهذه السياسة. تشمل التقنيات المحددة: رموز جلسة Clerk للمصادقة ومعرفات ذاكرة التخزين المؤقت المدعومة من Redis. *(ملاحظة: معالجة المدفوعات الذاتية معطلة حالياً ولا يتم تعيين أي ملفات دفع.)*'
      }
    },
    {
