...
<h1 className="text-2xl font-bold text-primary mb-2">{t({ en: 'Pricing information unavailable', ar: '??????? ??????? ??? ??????' })}</h1>
<p className="text-primary mb-2">{t({ en: "We couldn&apos;t load our pricing plans right now.", ar: '???? ????? ??? ??????? ??????.' })}</p>
<p className="text-sm text-primary/60 mb-8">{t({ en: 'Please try again or contact us for pricing details.', ar: '???? ???????? ??? ???? ?? ??????? ??? ?????? ??? ?????? ???????.' })}</p>
...
<a href="/contact" className="inline-flex min-h-[44px] items-center justify-center rounded-full border border-primary/20 px-6 py-3 text-sm font-semibold text-primary hover:bg-surface transition-all focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2">{t({ en: 'Contact us', ar: '???? ???' })}</a>
</div>
</div>
</main>
</div>
