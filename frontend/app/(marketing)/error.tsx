...
<div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-surface border border-primary/10"><AlertTriangle className="w-6 h-6 text-primary" />
          ...
          <h1 className="text-xl md:text-3xl font-bold text-primary mb-3">{t({ en: 'Something went wrong', ar: '??? ??? ??? ?????' })}</h1>
          <p className="text-sm text-primary mb-8 leading-relaxed">
            {t({ en: 'We encountered an error loading this page. Please try again.', ar: '???? ???????? ??? ???? ?? ?????? ??? ?????? ????????.' })}
          </p>
          ...
          {error.digest && (
            <p className="mt-6 text-xs text-primary font-mono">{t({ en: 'Error ID:', ar: '???? ?????:' })} {error.digest}</p>
          )}
        </div>
      </div>
    </div>
