  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <main className="mx-auto max-w-4xl px-6 py-20 lg:px-8 w-full text-center">
        ...
        <h1 className="text-2xl font-bold text-primary mb-2">{t({ en: 'Cookie policy unavailable', ar: '????? ????? ????? ???????? ??? ??????' })}</h1>
        <p className="text-primary mb-2">{t({ en: "We couldn&apos;t load the cookie policy.", ar: '???? ????? ????? ????? ????? ????????.' })}</p>
        <p className="text-sm text-primary/40 mb-8">{t({ en: 'Please try again.', ar: '???? ???????? ??? ????.' })}</p>
