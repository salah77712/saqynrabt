    ...
    <div className="bg-surface text-primary min-h-screen flex flex-col font-sans">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-20">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-primary mb-2">{t({ en: 'Case studies unavailable', ar: '?????? ?????? ??? ??????' })}</h1>
          <p className="text-primary mb-8">{t({ en: "We couldn&apos;t load the case studies. Please try again.", ar: '???? ????? ?????? ??????. ???? ???????? ??? ????.' })}</p>
